...
export async function POST(
  ...
  try {
    ...
    const newQuantityRaw = body?.newQuantity
    const noteRaw = body?.note

    const newQuantity = parseInt(newQuantityRaw, 10)
    if (isNaN(newQuantity) || newQuantity < 0 || newQuantity > 999999) {
      return NextResponse.json(
        { error: 'Invalid newQuantity. Must be between 0 and 999999' },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      const batchChange = qtyAfter - qtyBefore

      if (batchChange === 0) {
        await session.abortTransaction()
        return NextResponse.json(
          { error: 'No change: newQuantity is the same as current batch quantity' },
          { status: 400 }
        )
      }
      ...
      target.quantity = qtyAfter

      const recalculatedStock = batches.reduce((sum, b: any) => {
        const q = typeof b?.quantity === 'number' ? b.quantity : 0
        return sum + Math.max(0, q)
      }, 0)

      const stockAfter = recalculatedStock
        ...
        [
          {
            ...
            note: sanitizeString(`Batch ${batchId}: ${note}`, 500),
          },
        ],
        { session }
      )
      ...
      return NextResponse.json({
        ...
        batch: {
          ...
          quantity: typeof target?.quantity === 'number' ? target.quantity : 0,
          manufacturingDate: target?.manufacturingDate ? new Date(target.manufacturingDate).toISOString() : null,
          expirationDate: target?.expirationDate ? new Date(target.expirationDate).toISOString() : null,
          receivedAt: target?.receivedAt ? new Date(target.receivedAt).toISOString() : null,
        },
        product: {
